...
class TestKnowledgeBaseAgent(unittest.TestCase):
    ...
    def test_answer_uses_context_numbering_and_doc_id(self):
        prompt_capture = {}
        ...
        agent = self._make_agent()
        agent.llm_fn = fake_llm
        result = agent.answer("What is Python?", top_k=2)

        self.assertEqual(result, "Answer based on context.")
        self.assertIn("Instruction: chỉ dùng context; nói rõ khi context không đủ.", prompt_capture["prompt"])
        self.assertIn("Context:\n[1] doc_id=", prompt_capture["prompt"])
        self.assertIn("[2] doc_id=", prompt_capture["prompt"])
        self.assertIn("Question: What is Python?", prompt_capture["prompt"])
        self.assertTrue(prompt_capture["prompt"].strip().endswith("Answer:"))

    def test_answer_returns_warning_when_no_context(self):
        empty_store = EmbeddingStore(collection_name="kb_empty", embedding_fn=_mock_embed)
        agent = KnowledgeBaseAgent(store=empty_store, llm_fn=lambda prompt: "Should not call llm")
        result = agent.answer("Any question")

        self.assertIn("No relevant context found", result)
